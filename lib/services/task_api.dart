...
class TaskApi {
  ...
  static Stream<List<TodoModel>> getTasks() {
    try {
      ...
      final todos = FirebaseFirestore.instance.collection('todos').where("userId", isEqualTo: user!.uid).snapshots();
  ...
  static Future<TodoModel?> addTask(String task) async {
    try {
      ...
      final todo = TodoModel(userId: userId!, id: doc.id, text: task, isCompleted: false, createdAt: DateTime.now());
